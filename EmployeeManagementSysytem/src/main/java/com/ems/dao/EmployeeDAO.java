...
public class EmployeeDAO {
	...
	public boolean saveEmployee(Employee obj) {
		...
		String quary = "insert into employee(name,email,password,age,gender,mobile,department,address)values(?,?,?,?,?,?,?,?)";
		try {
 ...
 if(count == 0) {
	...
	public boolean editEmployee(Employee obj) {
		...
		connection = ConnectionManager.getConnection();
		String quary ="update employee set name=?,email=?,age=?,gender=?,mobile=?,department=?,address=? where id=?";
